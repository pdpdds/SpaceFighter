...
BOOL CPictureBox::OnRender(float fElapsedTime)
{
	...
	rcSrcRect.top = 0;
	rcSrcRect.right = 300;
	rcSrcRect.bottom = 300;

	vecCenter.x = .0f;
	vecCenter.y = .0f;
	vecCenter.z = .0f;

	vecPosition.x = 320.0f;
	vecPosition.y = 50.0f;
	vecPosition.z = .0f;

	m_pPictureSprite->Begin(D3DXSPRITE_ALPHABLEND);	
	m_pPictureSprite->Draw(m_pTextureGameTitle, &rcSrcRect, &vecCenter, &vecPosition, 0xffffffff);
	m_pPictureSprite->End();*/
