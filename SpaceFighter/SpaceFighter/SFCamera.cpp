...
CUserControlledCamera::CUserControlledCamera()
{
  ...
  m_vPosition = D3DXVECTOR3(60.0f,100.0f,-80.0f);
...
void CUserControlledCamera::Rotation(float fPitch, float fYaw, float fRoll)
{
	// Set the view matrix
	D3DXQUATERNION qR;
	D3DXQuaternionRotationYawPitchRoll(&qR, fPitch, fYaw, fRoll);
...
void CUserControlledCamera::Update(float fElapsedTime)
{
  ...
  FLOAT fAngularSpeed = 3.0f*fTime;
  ...
  m_fPitch += fAngularSpeed * m_fRollVelocity;
